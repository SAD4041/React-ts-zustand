...
import { FiHome, FiClipboard, FiUser, FiBarChart, FiSettings, FiShoppingBag, FiLogOut } from 'react-icons/fi';
...
export default function BrandSidebar() {
  const [isHovered, setIsHovered] = useState(false);
   ...
   const navItems: NavItem[] = [
    ...
    { id: 'products', label: 'محصولات', icon: <FiClipboard className="w-10 h-10" /> },
    { id: 'orders', label: 'سفارش‌ها', icon: <FiShoppingBag className="w-10 h-10" /> },
    { id: 'analytics', label: 'آمار و گزارش‌ها', icon: <FiBarChart className="w-10 h-10" /> },
    { id: 'profile', label: 'پروفایل', icon: <FiUser className="w-10 h-10" /> },
    { id: 'settings', label: 'تنظیمات', icon: <FiSettings className="w-10 h-10" /> },
